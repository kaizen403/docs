---
title: "Brainstorm"
description: "AI-powered task generation — describe a goal and get prioritized tasks you can insert directly into your board"
...
1. Open the Brainstorm panel from the sidebar or via [God Mode](/features/god-mode)
...
| Method | Action |
|--------|--------|
| Sidebar | Click "Brainstorm" in the sidebar |
| God Mode | Option+Space (macOS) or Ctrl+K (Windows/Linux) |
| Custom event | Dispatch a `brainstorm-query` DOM event from anywhere in the app |

The God Mode overlay dispatches a `brainstorm-query` custom event that the Brainstorm panel listens for, allowing any part of the UI to trigger task generation programmatically.
