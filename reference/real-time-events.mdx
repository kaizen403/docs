...
description: "Full SSE event catalog — every state change OpenLinear broadcasts to connected clients"
icon: "radio"
---
...
<Note>
Because SSE can't set request headers, authentication uses the `?token=<jwt>` query parameter instead of the `Authorization` header.
</Note>
...
| `task:updated` | full task object | Any task field changed |
