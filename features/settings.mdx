---
title: "Settings"
description: "Execution behavior and personal access token management"
icon: "sliders"
---
...
|---------|------|-------|---------|-------------|
| `parallelLimit` | integer | 1–5 | 3 | Maximum simultaneous tasks in single-task mode |
| `maxBatchSize` | integer | 1–10 | 3 | Maximum concurrent tasks in parallel batch mode |
| `queueAutoApprove` | boolean | — | false | Automatically start the next task in queue mode |
| `stopOnFailure` | boolean | — | false | Cancel remaining tasks in a batch if any task fails |
| `conflictBehavior` | string | skip, fail | skip | How to handle merge conflicts during batch merge |
...
Settings includes an **API Keys** section for personal access tokens (PATs). PATs authenticate MCP clients and API integrations without exposing the desktop session JWT.

OpenLinear shows a new token once and stores only its hash. The token list keeps metadata such as prefix, scopes, creation time, expiration time, last use, and revocation status.
...
See [MCP Integration](/integrations/mcp) for the MCP connection flow and PAT format.
